...
type state =
  ...
  ; phi : (N.RoleName.t * N.RoleName.t list) list (* sets of participants that have to be informed of a choice *)
  ; labels : N.LabelName.t list
  }

let is_enabled r st = List.find_opt (N.RoleName.equal r) st.psi |> Option.is_some
  ...
  let rec wf (g : G.t) : unit t =
    ...
    | G.ChoiceG (r, conts) ->
       ...
       let validate_branch g =
         ...
         let* _ = wf g in
         set st (* TODO: this is a reader monad more than a state monad *)
       in
